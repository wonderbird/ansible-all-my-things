...
- **FR-005**: The system MUST update `inventories/vagrant_tart_autogenerated.yml`
  (YAML format, groups `all` / `linux` / `vagrant_tart`) on both create and
  destroy. On create, the new entry MUST include `ansible_host` (VM IP),
  `ansible_port` (22), `ansible_user` (`admin`), and `ansible_ssh_pass` (the
  OCI image default credential from `playbooks/vars/tart_credentials.yml`). On
  destroy, all group references to the hostname MUST be removed.
