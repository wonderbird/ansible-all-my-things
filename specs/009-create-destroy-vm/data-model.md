...
Per-VM record in `inventories/tart_autogenerated.yml`. A VM is "known"
...
| `tart` | Selects Tart-specific vars |
...
```text
[absent]  →  create-vm.yml  →  [present in all + linux + tart]
[present] →  destroy-vm.yml →  [absent from all + linux + tart]
```
...
hostname (str)
    │  drives
    └──▶  InventoryEntry  ──written──▶  inventories/tart_autogenerated.yml
```
