...
Run the playbook a second time without changes. All `android_studio`
tasks — including SDK pre-provisioning — must report `ok` or `skipped`,
never `changed`.
...
All `android_studio` role tasks — including SDK pre-provisioning — must
be skipped; the playbook must complete without errors.
