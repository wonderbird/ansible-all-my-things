...
| Language pkg manager, per-user | `specify_cli` | `pipx install git+…@<tag>`, loop over `login_user_names` (assert it first — see SKILL.md step 1) | none | `shell creates:` on the per-user binary |
