...
`create-vm.yml -e provider=tart|docker -e profile=<basic|desktop>` followed by
`configure-profile.yml`, against the resulting `inventories/*_autogenerated.yml`.
Provider-specific defaults (e.g., `admin_user_on_fresh_system: root` for
Docker, `admin` for Tart) live in `inventories/group_vars/docker/` and
`inventories/group_vars/tart/`. See
...
- **Testing Integration**: Local Docker and Tart VMs provisioned via `create-vm.yml`
