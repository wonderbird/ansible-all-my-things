...
The project uses a dual-keyed-groups inventory strategy: each host belongs to
both a generic cross-provider group (`@linux`, `@windows`) and a
provider-specific group (`@hcloud_linux`, `@aws_ec2_linux`,
`@aws_ec2_windows`). This enables writing playbooks that target platforms
uniformly while retaining the ability to apply provider-specific overrides.
...
3. `provider_platform` (e.g., `hcloud_linux`, `aws_ec2_windows`) — provider +
   platform combination

The `platform:` host variable (e.g., `platform: "linux"`) is set via the
inventory `keyed_groups` configuration. It replaced the legacy
`ansible_group:` tag and provides backward-compatible cross-provider grouping.
