...
- [x] AWS Linux computers should be configured in the same way as the Hetzner VM. Try to map the provider-specific configuration completely using the corresponding provisioner script. If this is not possible: Do provider-specific inventory groups help, e.g., “hcloud_linux,” “aws_linux,” “aws_windows”?
  **Decision**: path-referenced defaults chosen over real `aws_linux`/`aws_windows` inventory groups. AMI/instance-type/region are create-time-only values consumed via `include_vars` from `playbooks/vars/` — creating inventory groups that the autogenerated inventory never produces would be misleading. No `aws_linux`/`aws_windows` groups are planned.
