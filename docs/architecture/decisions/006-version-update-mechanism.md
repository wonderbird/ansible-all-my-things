...
These are not negotiable within this decision. They eliminate options
before the drivers are applied.

- **C1 — Literal checksum pins must survive.** The repository must state
  the exact bytes it expects, as reviewable literals in
  `defaults/main.yml`. This is the same reasoning that already makes
  Principle IX SHA-pin third-party GitHub Actions: it defends against an
  upstream artefact being replaced after it was pinned — a retagged
  release, a force-pushed tag, a compromised account backfilling an old
  release. Under literal pins that fails loudly on every converge; under
  install-time verification against an upstream checksums file it does
  not, because whoever replaces the artefact replaces the checksums file
  in the same motion. Consequence: any candidate must be able to write
  checksums, not only version strings.
...
Listed in the priority established with the Product Owner.
...
2. **Marginal cost of adding a tool.** Registering a newly pinned tool
   should be a small, declarative, single-site change.
3. **Volume of code this project owns.** Every line of the update
   mechanism is infrastructure for the infrastructure. Less of it is
   better, provided the first driver still holds.
4. **Supply-chain integrity is preserved.** C1 must hold without becoming
   an unguarded manual step; a mechanism that bumps a version and leaves a
   stale digest behind is a regression, not a simplification.
5. **Fit with the existing Ansible and CI architecture.** The control node
...
- Good, because it removes the entire mechanism, its tests and its CI
  jobs at a stroke — the largest possible reduction against driver 3.
...
- Bad, because computing a per-architecture digest by hand for every tool
  ...
  which puts C1 at risk in practice even while honouring it on paper.
...
- Bad, and disqualifying: the hosted app does not execute
  ...
  a version Renovate has just bumped, so C1 cannot be met.
...
- Good, because what remains to own is small and sharply scoped: a Python
  ...
  `write-pins.yml` already implements (drivers 3 and 4).
- Good, because C1 is preserved in a stronger form than today: version and
  digests are produced by one function and written in one transaction, so
  the apply-order contract the Python checker currently polices becomes
  structurally impossible to violate, and roughly 720 lines of checker and
  checker tests are deleted rather than ported.
- Good, because it fits the existing architecture (driver 5): a Python
