...
`--extra-vars aws_ami_id=...`, `--extra-vars aws_instance_type=...`, `--extra-vars aws_region=...`):
...
`ansible-playbook playbooks/destroy-vm.yml --extra-vars provider=docker --extra-vars hostname=tatooine`.
...
`ansible-playbook playbooks/destroy-vm.yml --extra-vars provider=aws --extra-vars hostname=arrakis`.
