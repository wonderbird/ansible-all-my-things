...
**Hcloud differs**: run with `-e provider=hcloud`. Creates a disposable
Hetzner Cloud server using the default server type/location/image
(overridable via `-e hcloud_server_type=...`, `-e hcloud_server_location=...`,
`-e image=...`):
...
**AWS differs**: run with `-e provider=aws`. Ensures the shared `ansible-sg`
...
`-e aws_ami_id=...`, `-e aws_instance_type=...`, `-e aws_region=...`):
...
**Hcloud differs**: add `-e provider=hcloud`, e.g.
`ansible-playbook playbooks/destroy-vm.yml -e provider=hcloud -e hostname=edoras`.
...
**AWS differs**: add `-e provider=aws`, e.g.
`ansible-playbook playbooks/destroy-vm.yml -e provider=aws -e hostname=arrakis`.
